...
type Notifications_sms struct {
	Message string   `json:"message"`
	Phone   []string `json:"phone"`
}

func newApiezSmsClient(consumerKey, consumerSecret, auth, endpoint string) (*APIEZ, error) {
	token, err := getTokenApiez(consumerKey, consumerSecret, auth)
	if err != nil {
		return nil, err
	}
	return &APIEZ{
...
func getTokenApiez(consumerKey, consumerSecret, endpoint string) (string, error) {
...
func (e *APIEZ) SendMessage(param map[string]string, targetPhoneNumber ...string) error {
	objectN := Notifications_sms{
		Message: param["code"],
	...
	if resp.StatusCode != http.StatusOK {
		return fmt.Errorf("ApiezSmsClient's SendMessage() error, Apiez SMS request failed with status: %s", resp.Status)
	}
